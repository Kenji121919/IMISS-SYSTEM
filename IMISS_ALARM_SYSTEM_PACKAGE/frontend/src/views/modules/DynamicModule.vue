  ...
  <div class="page">
    ...
    <div class="topbar">
      ...
      <div class="topbar-actions">
        ...
        <button
          v-if="alarmBatchEnabled && selectedAlarmLogIds.length"
          class="btn-outline alarm-batch-top-btn"
          @click="openBatchAlarm"
          title="Set one alarm schedule for the selected logs"
        >
          <span class="btn-icon-left">🔔</span>
          Set alarm
          <span class="batch-count-badge">{{ selectedAlarmLogIds.length }}</span>
        </button>
        ...
        <table class="table" id="print-table">
          ...
          <thead>
            ...
            <tr>
              ...
              <th v-if="alarmBatchEnabled" class="alarm-select-th no-print">
                <input
                  type="checkbox"
                  :checked="allPageAlarmSelected"
                  :indeterminate.prop="somePageAlarmSelected"
                  title="Select this page for a batch alarm"
                  @change="togglePageAlarmSelection"
                />
              </th>
          ...
          <tbody>
            ...
            <tr v-if="pagedLogs.length === 0">
              ...
              <td :colspan="columns.length + 1 + (alarmBatchEnabled ? 1 : 0)" class="empty-row">
            ...
            >
              ...
              <td v-if="alarmBatchEnabled" class="alarm-select-cell no-print">
                <input
                  type="checkbox"
                  :checked="selectedAlarmLogIds.includes(Number(log.id))"
                  @change="toggleAlarmLogSelection(log.id)"
                  title="Select for batch alarm"
                />
              </td>
              ...
              <td class="td-actions no-print">
                ...
                <button
                  v-if="alarmIndividualEnabled"
                  class="action-btn alarm-action-btn"
                  @click.stop="openAlarmForLog(log)"
                  title="Set or edit alarm"
                >
                  🔔
                </button>
    ...
    <div v-if="showModal" class="modal-backdrop">
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <label
            v-if="!isEdit && alarmIndividualEnabled"
            class="alarm-after-save-option"
          >
            <input v-model="setAlarmAfterSave" type="checkbox" />
            <div>
              <strong>Set an alarm after adding this log</strong>
              <span>The alarm editor will open immediately after the record is saved.</span>
            </div>
          </label>
    ...
    <!-- ================= LOG ALARM EDITOR ================= -->
    <AlarmEditorModal
      :visible="showAlarmEditor"
      :module-id="route.params.id"
      :logs="alarmEditorLogs"
      :config="alarmConfig"
      @close="closeAlarmEditor"
      @saved="onAlarmSaved"
...
import AlarmEditorModal from '@/components/alarms/AlarmEditorModal.vue'
...
const alarmConfig = ref(defaultAlarmConfig())
const selectedAlarmLogIds = ref([])
const showAlarmEditor = ref(false)
const alarmEditorLogs = ref([])
const setAlarmAfterSave = ref(false)

const alarmIndividualEnabled = computed(() =>
  !!alarmConfig.value?.enabled &&
  ['individual', 'both'].includes(alarmConfig.value?.mode),
)

const alarmBatchEnabled = computed(() =>
  !!alarmConfig.value?.enabled &&
  ['batch', 'both'].includes(alarmConfig.value?.mode),
)
...
const loadAlarmConfig = async () => {
  try {
    const res = await api.get(
      `/alarms/module/${route.params.id}/config`,
    )
    ...
    alarmConfig.value = defaultAlarmConfig()
  }

  selectedAlarmLogIds.value = []
}

const openAlarmForLog = log => {
  if (!alarmIndividualEnabled.value || !log?.id) return
  alarmEditorLogs.value = [log]
  showAlarmEditor.value = true
}

const openBatchAlarm = () => {
  if (!alarmBatchEnabled.value || !selectedAlarmLogIds.value.length) return

  const selected = new Set(selectedAlarmLogIds.value.map(Number))
  alarmEditorLogs.value = logs.value.filter(log => selected.has(Number(log.id)))

  if (!alarmEditorLogs.value.length) {
    showToast('Select at least one log for the batch alarm', 'error')
    return
  }
...
const closeAlarmEditor = () => {
  ...
  alarmEditorLogs.value = []
}

const onAlarmSaved = async () => {
  const wasBatch = alarmEditorLogs.value.length > 1
  closeAlarmEditor()
  if (wasBatch) selectedAlarmLogIds.value = []
  showToast(wasBatch ? 'Batch alarms saved' : 'Alarm saved', 'success')
}
...
const allPageAlarmSelected = computed(() =>
  !!pagedLogs.value.length &&
  pagedLogs.value.every(log =>
    selectedAlarmLogIds.value.includes(Number(log.id)),
  ),
)

const somePageAlarmSelected = computed(() =>
  pagedLogs.value.some(log =>
    selectedAlarmLogIds.value.includes(Number(log.id)),
  ) && !allPageAlarmSelected.value,
)

const toggleAlarmLogSelection = logId => {
  const id = Number(logId)
  if (!id) return

  if (selectedAlarmLogIds.value.includes(id)) {
    selectedAlarmLogIds.value = selectedAlarmLogIds.value.filter(item => item !== id)
  } else {
    selectedAlarmLogIds.value = [...selectedAlarmLogIds.value, id]
  }
}

const togglePageAlarmSelection = () => {
  const pageIds = pagedLogs.value.map(log => Number(log.id)).filter(Boolean)

  if (allPageAlarmSelected.value) {
    selectedAlarmLogIds.value = selectedAlarmLogIds.value.filter(
      id => !pageIds.includes(id),
    )
  } else {
    selectedAlarmLogIds.value = Array.from(
      new Set([...selectedAlarmLogIds.value, ...pageIds]),
    )
  }
}
...
const saveLog = async () => {
  ...
  const shouldSetAlarm = !wasEdit && setAlarmAfterSave.value && alarmIndividualEnabled.value
  let savedLog = null
  ...
  try {
    ...
    if (wasEdit) {
      ...
      const res = await api.put(`/logs/${selectedId.value}`, { data: form.value, ...meta })
      savedLog = res.data ? normalizeLog(res.data) : null

    } else {

      const res = await api.post('/logs', { moduleId: route.params.id, data: form.value, ...meta })
      savedLog = res.data ? normalizeLog(res.data) : null
    ...
    showToast(wasEdit ? 'Log updated' : 'Log added', 'success')

    if (shouldSetAlarm && savedLog?.id) {
      const freshSaved = logs.value.find(log => Number(log.id) === Number(savedLog.id)) || savedLog
      await nextTick()
      openAlarmForLog(freshSaved)
    }
...
.alarm-select-th input,
.alarm-select-cell input {
  width: 15px;
  height: 15px;
  cursor: pointer;
  accent-color: #2563eb;
...
.alarm-after-save-option input {
  margin-top: 2px;
  accent-color: #2563eb;
}
.alarm-after-save-option strong,
.alarm-after-save-option span {
  display: block;
}
.alarm-after-save-option strong {
  color: #92400e;
